import Paragraph from "@/components/typography/Paragraph";
import { imageBlur } from "@/lib/utils";
import AppCta from "@/widgets/app/cta";
import AppTextBody from "@/widgets/app/text-body";
import Image from "next/image";

const checklist = [
  "Travel the world with ease",
  "Over 300+ airlines and hotels",
  "Book flight and pay later",
  "Explore more",
];

const HomeLifestyleSection = () => {
  return (
    <section id="lifestyle-section" className="bg-[#E9E9E9]/30 py-24">
      <div className="content-container">
        <AppTextBody
          containerId="lifestyle-section"
          title="Get the lifestyle you deserve with Platnova"
          description={
            <div className="space-y-10">
              <Paragraph size="small">
                With just a few clicks, you can explore the world the way you
                want with Platnova
              </Paragraph>

              <ul className="grid md:justify-end gap-4">
                {checklist.map((item, index) => (
                  <li
                    key={`checklist-${index}`}
                    className="flex items-center md:justify-end gap-3"
                  >
                    <svg
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                      xmlnsXlink="http://www.w3.org/1999/xlink"
                    >
                      <rect
                        width="24"
                        height="24"
                        fill="url(#pattern0_2244_2674)"
                      />
                      <defs>
                        <pattern
                          id="pattern0_2244_2674"
                          patternContentUnits="objectBoundingBox"
                          width="1"
                          height="1"
                        >
                          <use
                            xlinkHref="#image0_2244_2674"
                            transform="scale(0.0078125)"
                          />
                        </pattern>
                        <image
                          id="image0_2244_2674"
                          width="128"
                          height="128"
                          xlinkHref="data:image/png;base64,"
                        />
                      </defs>
                    </svg>
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
              <div>
                <AppCta text="Learn more" link="/lifestyle" />
              </div>
            </div>
          }
        />

        <div className="w-full md:w-1/2 aspect-square grid grid-cols-2 gap-3 lg:gap-5 min-h-[300px] md:min-h-[auto] md:max-h-[350px] lg:max-h-[400px] xl:max-h-[520px] md:-mt-40 mt-10 relative ">
          <div data-scroll data-scroll-speed="2" className="relative h-full">
            <Image
              src="/airplane.png"
              alt=""
              fill
              sizes="(max-width: 640px) 60vw, 30vw"
              {...imageBlur}
            />
          </div>
          <div data-scroll-speed="-1" className="relative h-full">
            <Image
              src="/man-smiling.png"
              alt=""
              fill
              sizes="(max-width: 640px) 60vw, 30vw"
              {...imageBlur}
            />
          </div>

          <div className="absolute left-full xl:bottom-auto bottom-16 lg:bottom-20 pl-5 lg:pl-16 hidden md:block">
            <svg
              width="190"
              height="157"
              viewBox="0 0 190 157"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clipPath="url(#clip0_1496_1830)">
                <path
                  d="M166.483 3.45802C161.509 26.2206 164.412 10.0719 157.578 43.1943C150.745 76.3167 96.2046 75.6277 102.913 58.1233C110.797 37.5521 138.859 35.6109 136.05 56.5985C130.552 97.6751 75.5303 96.0515 75.5303 96.0515C75.5303 96.0515 60.3704 95.084 54.9459 92.3481C49.4772 89.5898 45.3265 85.6121 45.8217 79.4256C46.7063 68.3754 65.0078 62.3185 75.0928 66.2691C88.5644 71.5462 85.9182 92.9166 71.5175 110.934C61.2438 123.789 24.1895 133.8 24.1895 133.8"
                  stroke="black"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeDasharray="8 8"
                />
                <path
                  d="M1.55193 153.424C1.35958 153.193 1.23703 152.911 1.19856 152.612C1.1601 152.314 1.2073 152.01 1.33467 151.737L16.7037 118.994L19.6776 118.994L25.1787 132.747L38.7506 141.569L38.7506 144.543L3.17248 153.942C2.88111 154.018 2.57409 154.01 2.28718 153.918C2.00027 153.826 1.74529 153.655 1.55193 153.424ZM35.9141 142.707L24.2483 135.124L11.4684 146.792L9.78279 144.946L22.6622 133.187L18.1265 121.847L4.43185 151.022L35.9141 142.707Z"
                  fill="black"
                />
              </g>
              <defs>
                <clipPath id="clip0_1496_1830">
                  <rect
                    width="156.358"
                    height="189.55"
                    fill="white"
                    transform="matrix(-4.37114e-08 1 1 4.37114e-08 0 0)"
                  />
                </clipPath>
              </defs>
            </svg>
          </div>
        </div>
      </div>
    </section>
  );
};

export default HomeLifestyleSection;
